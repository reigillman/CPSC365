...
echo("<form action='userprofileinput.php?userId=".$_SESSION['userId']."' enctype='multipart/form-data' method='POST'>
	...
	else if(($getUser['avatar'] != null) && $_SESSION['userId'] == $getUser['userId']){
		echo ("<img src= 'avatars/".$getUser['avatar']."(thumbnail).png' >");
		echo("<form action='changeprofileinput.php?userId=".$_SESSION['userId']."' enctype='multipart/form-data' method='POST'>
	<input type='file' name='upload' accept='.png'>
...
while($getFriends= $userFriends->fetch()){
	if((($getFriends['status'] == 'accepted') && $getFriends['fromUser'] == $user) && $getFriends['toUser'] != $user){
	...
	else if((($getFriends['status'] == 'accepted') && $getFriends['toUser'] == $user) && $getFriends['fromUser'] != $user){
